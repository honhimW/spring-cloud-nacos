...
import static io.github.honhimw.nacos.api.PropertyKeyConst.*;
...
public class NacosDiscoveryProperties {
	...
	public void init() throws Exception {
		...
		if (!StringUtils.hasText(ip)) {
			...
			if (!StringUtils.hasText(networkInterface)) {
				if (ipType == null) {
	...
	public void overrideFromEnv(Environment env) {
		...
		if (!StringUtils.hasText(this.getServerAddr())) {
		...
		if (!StringUtils.hasText(this.getNamespace())) {
		...
		if (!StringUtils.hasText(this.getLogName())) {
			this.setLogName(
					env.resolvePlaceholders("${spring.cloud.nacos.discovery.log-name:}"));
		}
		if (!StringUtils.hasText(this.getClusterName())) {
		...
		if (!StringUtils.hasText(this.getGroup())) {
			this.setGroup(
