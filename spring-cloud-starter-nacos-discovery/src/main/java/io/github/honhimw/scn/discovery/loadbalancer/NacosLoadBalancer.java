...
package io.github.honhimw.scn.discovery.loadbalancer;
		...
		if (StringUtils.hasText(ipv6)) {
			...
			for (ServiceInstance instance : instances) {
				if (Pattern.matches(IPV4_REGEX, instance.getHost())) {
					if (StringUtils.hasText(instance.getMetadata().get(IPV6_KEY))) {
				...
				List<ServiceInstance> sameClusterInstances = serviceInstances.stream()
						.filter(serviceInstance -> {
							String cluster = serviceInstance.getMetadata()
									.get("nacos.cluster");
							return ObjectUtils.nullSafeEquals(cluster, clusterName);
						}).collect(Collectors.toList());
